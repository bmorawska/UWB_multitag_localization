...
class ReadyToRange(object):
    ...
    def __init__(self, pozyx, tag_ids, range_step_mm=1000, protocol=PozyxConstants.RANGE_PROTOCOL_PRECISION,
                 remote_id=None):
        self.pozyx = pozyx
    ...
    def setup(self):
        ...
        if self.remote_id is None:
            for tag in self.tag_ids:
                for device_id in [self.remote_id, tag]:
                    self.pozyx.printDeviceInfo(device_id)
        else:
            for tag in self.tag_ids:
                for device_id in [None, self.remote_id, tag]:
                    self.pozyx.printDeviceInfo(device_id)
        print("")
        print("- -----------POZYX RANGING V{} -------------".format(version))
        print("")
    ...
    def loop(self):
        ...
        for tag in self.tag_ids:
            ...
            status = self.pozyx.doRanging(
                tag, device_range, self.remote_id)
            if status == POZYX_SUCCESS:
                msg = f'"timestamp": {int(time() * 1000)}, "device_timestamp":{device_range.data[0]}, "tag_id":{"0x%0.4x" % tag} "rssi":{device_range.data[2]}, "distance":{device_range.data[1]}, "status":"success"'
                print(msg)
                ...
                if status == POZYX_SUCCESS:
                    print(f'"timestamp":{int(time() * 1000)},"status":"failed","tag_id":{"0x%0.4x" % tag},"message": "Error - {self.pozyx.getErrorMessage(error_code)}"')
...
if __name__ == "__main__":
    ...
    #destination_id = 0x6757      # network ID of the ranging destination
    ...
    r = ReadyToRange(pozyx, tag_ids, range_step_mm,
